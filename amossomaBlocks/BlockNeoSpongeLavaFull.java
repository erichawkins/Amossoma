...
public class BlockNeoSpongeLavaFull extends Block
{
	protected BlockNeoSpongeLavaFull(int id, int texture, Material materials)
	{
	...
	{   	
        {
    		this.SploogeLava(par1World, par2, par3, par4);
            return true;
        }
	}

	private void SploogeLava(World par1World, int par2, int par3, int par4)
	{ 	
		if (AmossomaAirPhysics.isAir(par1World, par2 + 1, par3, par4))
			par1World.setBlock(par2 + 1, par3, par4, Block.lavaStill.blockID);
		if (AmossomaAirPhysics.isAir(par1World, par2 , par3, par4 + 1))
			par1World.setBlock(par2, par3, par4 + 1, Block.lavaStill.blockID);
		if (AmossomaAirPhysics.isAir(par1World, par2 - 1, par3, par4))
			par1World.setBlock(par2 - 1, par3, par4, Block.lavaStill.blockID);
		if (AmossomaAirPhysics.isAir(par1World, par2, par3, par4 - 1))
			par1World.setBlock(par2, par3, par4 - 1, Block.lavaStill.blockID);
		...
		if (AmossomaAirPhysics.isAir(par1World, par2, par3 - 1, par4))
			par1World.setBlock(par2, par3 - 1, par4, Block.lavaStill.blockID);
		this.LavaEmptied(par1World, par2, par3, par4);
	}
	
	
	private void LavaEmptied(World par1World, int par2, int par3, int par4)
	{
		par1World.setBlockWithNotify(par2, par3, par4, AmossomaBlocks.neoSpongeLavaEmpty.blockID);
	}
