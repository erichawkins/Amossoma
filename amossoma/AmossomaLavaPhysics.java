...
@Mod(modid = "Ash_AmossomaLavaPhysics", name = "AmossomaLavaPhysics", version = "1.0")
@NetworkMod(clientSideRequired = true, serverSideRequired = false)
public class AmossomaLavaPhysics 
{
	...
	/**
	 * Checks to see if the block is lava. Args: world, x, y, z 
	 */
	public static boolean isLava(World par1World, int par2, int par3, int par4)
	{
		int var11 = par1World.getBlockId(par2, par3, par4);

		if (var11 == Block.lavaMoving.blockID)
		{
			return true;
		}
		else if (var11 == Block.lavaStill.blockID)
		{
			return true;
		}
		else
	    ...
	    /**
	     * Checks to see if the block is NOT next to an lava block. Args: world, x, y, z 
	     * 
	     * True = Water Does Not Touch Block
	     * False = Water Touches Block
	     */
		
	    public static boolean isNotExposedtoLava(World par1World, int par2, int par3, int par4)
	    {
	    ...
	    /**
	     * Checks to see if the block is next to an lava block. Args: world, x, y, z 
	     * 
	     * True = Water Touches Block
	     * False = Water Does Not Touch Block
	     */
	    
	    public static boolean isExposedtoLava(World par1World, int par2, int par3, int par4)
	    {
	    	Block block = Block.blocksList[par1World.getBlockId(par2, par3, par4)];
	    	int var12 = par1World.getBlockId(par2, par3 + 1, par4);
	    	int var13 = par1World.getBlockId(par2, par3 - 1, par4);
		   ...
		   /**
		     ...
		     * @return
		     */
		    public static boolean CanFloatAboveLava(World par1World, int par2, int par3, int par4, int par5)
		    {
		        //do not rise further than max height
		    	if (par3 >= par5) return false;
		    	int var4 = par1World.getBlockId(par2, par3, par4);

		    	if (var4 == Block.lavaMoving.blockID)
		    	{
		    		return true;
		    	}
		    	else if (var4 == Block.lavaStill.blockID)
		    	{
		    		return true;
		    	}
		    	else if (var4 == 0)
